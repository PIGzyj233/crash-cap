...
#[test]
fn analysis_context_owns_final_identity_time_and_engine_facts() {
    let dump = unique_path("context-final", "dmp");
    let context_path = unique_path("context-final", "json");
    let output = unique_path("context-final-output", "json");
    let raw_dir = unique_path("context-final-raw", "dir");
    let bytes = minimal_x64_dump();
    fs::write(&dump, &bytes).expect("write dump");
    let digest = dmp_core::canonical::sha256_hex(&bytes);
    let inspect = dmp_core::inspect_bytes(&bytes).expect("inspect context fixture");
    let inspect_value = serde_json::to_value(&inspect).expect("serialize inspect value");
    let inspect_digest = dmp_core::canonical::sha256_hex(
        &serde_json::to_vec(&inspect_value).expect("serialize inspect bytes"),
    );
    let zero_digest = format!("sha256:{}", "0".repeat(64));
    let context = serde_json::json!({
        "schema_version": "analysis-context-v1",
        "identity": {
            "workspace_id": "wsp_context",
            "occurrence_id": "occ_context",
            "analysis_id": "run_context"
        },
        "dump": {
        ...
        "engine": {
            "core_image_digest": zero_digest,
            "symbolicator_version": "test-symbolicator",
            "grouping_version": "group-v1.0",
            "normalization_version": "norm-v1.0"
        },
        "policy": {
            "symbol_inventory_version": 7,
            "source_bundle_policy_version": "source-bundle-v1.0"
        },
        "inspect": {"sha256": inspect_digest},
        "inputs": {"source_bundles": []}
    });
    fs::write(&context_path, serde_json::to_vec(&context).expect("serialize context"))
    ...
    let result = Command::new(env!("CARGO_BIN_EXE_dmp-core"))
        .args([
            ...
            "test-symbolicator",
            "--analysis-context",
            context_path.to_str().expect("UTF-8 temp path"),
            "--output",
            output.to_str().expect("UTF-8 temp path"),
            "--raw-dir",
            raw_dir.to_str().expect("UTF-8 temp path"),
    ...
    let canonical: Value = serde_json::from_slice(&fs::read(&output).expect("read canonical"))
        .expect("canonical JSON");
    assert_eq!(canonical["workspace_id"], "wsp_context");
    assert_eq!(canonical["occurrence_id"], "occ_context");
    assert_eq!(canonical["analysis_id"], "run_context");
    assert_eq!(canonical["dump"]["blob_id"], "blob_context");
    assert_eq!(canonical["dump"]["time_source"], "reported");
    assert_eq!(canonical["dump"]["occurred_at"], "2025-01-01T00:00:00+00:00");
    assert!(raw_dir.join("legacy-canonical.json").is_file());
    cleanup(&[&dump, &context_path, &output]);
    let _ = fs::remove_dir_all(&raw_dir);
}
