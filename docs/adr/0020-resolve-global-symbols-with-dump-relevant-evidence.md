...
The trade-off is an explicit catalog and planning lifecycle rather than broad Workspace-version invalidation or simply removing Workspace filters. Storage, idempotency, cache invalidation, source paths, input selection, immutable Runs, and affected-demand dispatch must agree at enablement. The [implementation design](../qa-symbol-import-implementation-design.md) records the data model, deterministic algorithm, and defaults; the [implementation guide](../qa-symbol-import-guide.md) maintains pending real-Symbolicator, concurrency, and rollout gates. Compatible stages may be delivered separately before enabling the complete feature.
