...
登录后的全局顶栏提供个人账号菜单和直接可见的“退出登录”；首次设置密码页面也可以退出。退出请求期间各入口共享处理中状态，服务端确认成功或会话已失效后清理本地身份并同步其他标签页，仍在当前地址显示登录界面。网络或服务端失败会提示重试，不会仅凭前端清理宣称注销成功。

退出使原账号的后续上传步骤和轮询失效，已开始的对象传输可以自然结束，服务端已经受理的验收与分析任务继续处理。上传收据按用户 ID 分区保留，重新登录后可以恢复查看；旧回调不能改写新登录的页面或收据。
