...
浏览器 Workspace 上传页默认当前空间；平台上传页可选择空间或公共区域，整批版本可选。符号中心的文件库展示文件可用性，符号问题列表展示当前报告的实际影响，报告允许明确编辑 DMP 的当前版本。
...
上传现在必须登录或配置平台上传令牌。请按[认证上线步骤](authentication.md#上线与回滚)执行增量迁移、管理员初始化、Origin 配置和 CI 令牌签发。HTTP 内网部署不要求 HTTPS 证书。
