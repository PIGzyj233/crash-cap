...
describe('configurable /api/v3 client', () => {
  it.each(['2.0'])('reads Canonical %s through the v3 reader', async (schema_version) => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValue(new Response(JSON.stringify({ schema_version }), { status: 200 }))
    const api = createApiClient({ baseUrl: 'http://localhost:8000/api/v3/', fetcher })
    await expect(api.getOccurrenceAnalysis('occ/1', 'run/2')).resolves.toEqual({ schema_version })
    expect(fetcher).toHaveBeenCalledWith('http://localhost:8000/api/v3/occurrences/occ%2F1/analysis?run_id=run%2F2', expect.anything())
  })
  ...
  it('supports a separately configured analysis proxy for all sections', async () => {
    ...
    const api = createApiClient({ baseUrl: '/reader/v3', fetcher })
    await api.getOccurrenceThreads('occ_1')
    await api.getOccurrenceModules('occ_1')
    expect(fetcher.mock.calls.map(([url]) => url)).toEqual(['/reader/v3/occurrences/occ_1/threads', '/reader/v3/occurrences/occ_1/modules'])
  })

  it('reads capabilities and declares an exact Workspace role through v2', async () => {
    const fetcher = vi.fn<typeof fetch>()
      .mockResolvedValueOnce(new Response(JSON.stringify({ reader_versions: ['2.0'], enabled_writes: ['workspace_module_roles'], pause_reason: null }), { status: 200 }))
    ...
    expect(fetcher).toHaveBeenNthCalledWith(1, '/reader/v3/capabilities', expect.anything())
    expect(fetcher).toHaveBeenNthCalledWith(2, '/reader/v3/workspaces/wsp%2Ftest/module-roles', expect.objectContaining({ method: 'POST', body: JSON.stringify(input) }))
  })
  ...
  it('uses the configured API prefix and JSON request body', async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith('http://localhost:8000/api/v3/workspaces', expect.objectContaining({ headers: expect.any(Headers) }))
  })

  it('does not expose a raw URL when downloads are disabled', async () => {
    const api = createApiClient({ baseUrl: '/api/v3', rawDownloadEnabled: false, fetcher: vi.fn() })
  ...
  it('requests a PE/PDB artifact presigned URL only when raw downloads are enabled', async () => {
    ...
    const api = createApiClient({ baseUrl: '/api/v3', rawDownloadEnabled: true, fetcher })
    ...
    expect(fetcher).toHaveBeenCalledWith('/api/v3/artifacts/art%2Fpe/download', expect.objectContaining({ headers: expect.any(Headers) }))
  })
  ...
  it('builds an SSE endpoint and submits a targeted batch reprocess', async () => {
    ...
    const api = createApiClient({ baseUrl: 'http://localhost:8000/api/v3', fetcher })
    expect(api.getOccurrenceEventsUrl('occ/1')).toBe('http://localhost:8000/api/v3/occurrences/occ%2F1/events')
    await api.batchReprocessSymbols('wsp_test', { occurrence_ids: ['occ_1'] })
    expect(fetcher).toHaveBeenCalledWith('http://localhost:8000/api/v3/workspaces/wsp_test/symbols/reprocess', expect.objectContaining({ method: 'POST', body: JSON.stringify({ occurrence_ids: ['occ_1'] }) }))
  })
})
