  ...
  return <>
    ...
    <Modal open={open} onCancel={() => setOpen(false)} footer={null} title="配对来源与提供方记录" width={900}>
      <Space direction="vertical" style={{ width: '100%' }}>
        <Typography.Text>来源记录不代表当前 Workspace 的模块角色。身份冲突需要提供方核对文件并提供复核依据。</Typography.Text>
        ...
        <Table rowKey="id" dataSource={rows} pagination={false} loading={query.isFetching && !query.isFetchingNextPage} locale={{ emptyText: query.data ? '未记录来源' : '尚未取得来源记录' }} columns={[
          ...
          { title: '产物', render: (_, row) => <Link to={row.source_workspace_id ? `/w/${encodeURIComponent(row.source_workspace_id)}/artifacts` : '/artifacts'} onClick={() => setOpen(false)}>查看产物与符号</Link> },
        ]} />
