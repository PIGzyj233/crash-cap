...
function restoreRequest(key: string): { request: Review | null; warning: string | null } {
  try {
    ...
    if (!Number.isSafeInteger(value.expected_version) || Number(value.expected_version) < 1
      ...
      || !(['reason', 'evidence', 'idempotency_key'] as const).every((field) => typeof value[field] === 'string' && value[field]!.trim())) throw new Error('Invalid saved request')
...
export function CatalogReviewForm({ pairId, version, onSaved }: { pairId: string; version: number; onSaved: () => void }) {
  ...
  const storageKey = `crashcap.review.pending.v2:${currentUser()?.id ?? "test"}:${import.meta.env.VITE_API_BASE_URL ?? '/api/v1'}:${pairId}`
  ...
  return <Space direction="vertical" style={{ width: '100%' }}>
    <Alert type="warning" showIcon message="提供方复核影响全平台对此配对的后续使用" description="停用保留文件和历史报告；恢复不保证身份冲突已经消除。请填写实际核查依据。复核人由当前登录账号自动记录。" />
    ...
    <Button loading={busy} disabled={!enabled || saved || !confirmed || !reason.trim() || !evidence.trim()} onClick={() => void submit()}>{error ? '重试相同复核' : '提交复核结论'}</Button>
  </Space>
}
