...
export function DemandRestartForm({ workspaceId, occurrenceId, demand, onSaved }: Props) {
  ...
  const submit = async () => {
    if (sending.current || saved || rejected || initial.error || (!pending.current && (!enabled || demand?.state !== 'retry_exhausted'))) return
    const body = pending.current ?? { idempotency_key: createUuid(), expected_generation: demand!.generation, expected_sequence: demand!.change_sequence, rationale: rationale.trim() || '用户请求重新分析' }
  ...
  return <Space direction="vertical" style={{ width: '100%' }}>
    ...
    <Input.TextArea aria-label="重新分析说明（可选）" placeholder="可选：补充已处理的问题或重新分析的原因" maxLength={2000} value={rationale} disabled={busy || saved || !!pending.current || !enabled} onChange={(event) => setRationale(event.target.value)} />
    ...
    <Button type="primary" loading={busy} disabled={saved || rejected || !!initial.error || (!pending.current && (!enabled || demand?.state !== 'retry_exhausted'))} onClick={() => void submit()}>{saved ? '请求已受理' : pending.current ? '确认此前重开请求' : '请求重新分析'}</Button>
  </Space>
}
