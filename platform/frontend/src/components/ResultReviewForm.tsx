...
type Review = components['schemas']['ResultReviewInput']
...
function storageKey({ workspaceId, occurrenceId, candidateRunId }: Pick<Props, 'workspaceId' | 'occurrenceId' | 'candidateRunId'>) {
  return `crashcap.result-review.v2:${currentUser()?.id ?? "test"}:${import.meta.env.VITE_ANALYSIS_API_BASE_URL ?? import.meta.env.VITE_API_BASE_URL ?? '/api/v1'}:${workspaceId}:${occurrenceId}:${candidateRunId}`
}
...
function restore(key: string, candidate: string): Review | null {
  ...
  if (value.schema_version !== 'result-review-request-v1' || value.candidate_run_id !== candidate
    || !value.current_run_id || !value.idempotency_key || !value.rationale?.trim()
...
function FormBody({ workspaceId, occurrenceId, currentRunId, candidateRunId, onSaved }: Props) {
  ...
  return <Space direction="vertical" style={{ width: '100%' }}>
    ...
    <Typography.Text type="secondary">审核人由当前登录账号自动记录。审核将绑定这两份报告，提交时服务端会重新检查当前报告及证据。</Typography.Text>
    ...
    <Button type="primary" loading={busy} disabled={!enabled || saved || !confirmed || !rationale.trim() || (cause === 'evidence_correction' && !basis.length) || basis.some((item) => !item.review_id.trim() || !/^[a-f0-9]{64}$/.test(item.evidence_sha256))} onClick={() => void submit()}>{saved ? '审核已保存' : pending.current ? '重试相同报告审核' : '提交报告审核'}</Button>
  </Space>
}
