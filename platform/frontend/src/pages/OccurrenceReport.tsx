...
export function OccurrenceReport({ workspace, occurrenceId, onBack, onOpenGroup }: { workspace: Workspace; occurrenceId: string; onBack: () => void; onOpenGroup: (groupId: string) => void }) {
  ...
  if (terminal && !successful) {
    ...
    return <div>{demandStatus}<OccurrenceVersionEditor occurrence={occurrence} /><Link className="back-button" to={routePaths.occurrences(workspace.id)}><ArrowLeftOutlined /> 返回 Crash Inbox</Link><Card className="analysis-progress-card"><Alert type="error" showIcon message={stagingFailure ? '分析输入准备失败' : `分析${statusLabel(current?.status)}`} description={current?.error_detail ?? current?.error_code ?? '分析未生成可展示结果'} /><Space wrap><StatusTag status={current?.status ?? 'FAILED'} /><HashValue value={current?.id} />{!demand.data && <Button type="primary" icon={<ReloadOutlined />} disabled={demand.isPending || demand.isError} loading={reprocess.isPending} onClick={() => reprocess.mutate()}>重新分析</Button>}</Space><Text type="secondary">{demand.data ? '请通过上方分析更新状态查看进展；自动重试耗尽后可填写原因请求重新分析。原失败 Run 会保留作为历史证据。' : '重新分析会创建新的 Analysis Run，原失败 Run 会保留作为历史证据。'}</Text></Card></div>
  }
  ...
  const tabItems = [
    ...
    { key: 'raw', label: 'Raw Metadata', children: <Card><pre className="json-block">{JSON.stringify({ dump: result.dump, process: result.process, engine: result.engine }, null, 2)}</pre><Alert type="info" showIcon message="此处是 Canonical metadata 摘要，不是原始内存转储。" /></Card> },
